    ...
    <header ref={headerRef} className="sticky top-0 z-50 shadow-md">
      ...
      <div className="bg-white border-b border-gray-200">
            ...
            <div className="relative" ref={userMenuRef}>
              ...
              {userMenuOpen && user && (
                <div className="absolute right-0 top-full z-50 mt-2 w-60 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-xl">
                  ...
                  <div className="p-1">
                    <Link to="/account" onClick={() => setUserMenuOpen(false)} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-gray-800 hover:bg-gray-50">
                      <User className="h-4 w-4 text-gray-500" /> Il mio account
                    </Link>
                    {isAdmin && (
                      <Link to="/admin" onClick={() => setUserMenuOpen(false)} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-gray-800 hover:bg-gray-50">
                        <Shield className="h-4 w-4 text-gray-500" /> Pannello Admin
                      </Link>
                    )}
        ...
        <div className="px-5 pb-3 sm:hidden">
          <div className="relative">
            <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
            <input type="text" placeholder="Cerca nel catalogo" className="h-10 w-full rounded-full border border-gray-300 bg-gray-50 pl-9 pr-[80px] text-sm placeholder:text-gray-400 focus:border-green-500 focus:outline-none focus:ring-2 focus:ring-green-100" />
            <button className="absolute right-1 top-1 bottom-1 rounded-full bg-green-600 px-4 text-[13px] font-bold text-white">Cerca</button>
          </div>
        </div>
      </div>
      ...
      {/* ── MEGA MENU: dark bg, columns (identico a Tendina menu.png) ── */}
      {activeCatalog && (
        <div
              ...
              {activeCatalog.groups.map((group) => (
                <div key={group.slug} className="min-w-[150px]">
                  <Link
                    ...
                    onClick={() => setActiveCatalogKey(null)}
                    className="block text-[13px] font-bold text-white mb-3 hover:text-green-400 transition-colors"
                  >
                    {group.title}
                  </Link>
