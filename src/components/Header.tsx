...
export default function Header({ cartItemCount, onCartClick, favoritesCount, onFavoritesClick, onAccountClick, onLogoClick, user, currentPage, onNavigate }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-300 bg-white shadow-[0_2px_12px_rgba(15,23,42,0.10)]">
                ...
                <div className="absolute right-0 top-full mt-2 w-64 bg-white rounded-xl shadow-xl border border-gray-200 overflow-hidden z-50">
                  <div className={`p-3 ${isPro ? 'bg-amber-50' : isAdmin ? 'bg-slate-50' : 'bg-emerald-50'}`}>
                    <div className="flex items-center gap-2">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold ${isPro ? 'bg-amber-500' : isAdmin ? 'bg-slate-700' : 'bg-emerald-500'}`}>
                  ...
                  <div className="p-1">
                    <button onClick={() => { onNavigate('account'); setUserMenuOpen(false); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 rounded-lg transition-colors">
                      <User className="w-4 h-4 text-gray-400" /> Il mio account
                    </button>
                    <button onClick={() => { onNavigate('wallet'); setUserMenuOpen(false); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 rounded-lg transition-colors">
                      <Wallet className="w-4 h-4 text-gray-400" /> Wallet
                      <span className="ml-auto text-xs font-bold text-emerald-600">{user.walletBalance.toFixed(2)}</span>
                    </button>
                    {isAdmin && (
                      <button onClick={() => { onNavigate('admin'); setUserMenuOpen(false); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 rounded-lg transition-colors">
                        <Shield className="w-4 h-4 text-gray-400" /> Pannello Admin
                      </button>
                    )}
                    <div className="border-t border-gray-100 mt-1 pt-1">
                      <button onClick={() => { logout(); setUserMenuOpen(false); onNavigate('home'); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-red-600 hover:bg-red-50 rounded-lg transition-colors">
            ...
            {catalogMenu.map(({ key, label }) => {
              ...
              return (
                <li key={key} onMouseEnter={() => setActiveCatalogKey(key)}>
                  <button type="button" onClick={() => onNavigate('home')} className="inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-semibold text-slate-100 hover:bg-slate-700 transition-colors whitespace-nowrap">
      ...
      {activeCatalog && (
        <div className="hidden md:block border-t border-slate-200 bg-white shadow-lg" onMouseLeave={() => setActiveCatalogKey(null)}>
